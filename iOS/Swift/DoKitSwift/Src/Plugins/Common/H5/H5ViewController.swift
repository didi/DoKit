...
class H5ViewController: BaseViewController {
    ...
    // MARK: - Target Methods
    @objc func clickScanBtnClickHandle(sender: UIButton) -> Void {
        print("扫码...")
    }
    
    @objc func jumpBtnClickHandle(sender: UIButton) -> Void {
...
extension H5ViewController {
    func initUI() -> Void {
        ...
        jumpBtn.addTarget(self, action: #selector(jumpBtnClickHandle(sender:)), for: .touchUpInside)
        ...
        scanJumpBtn.addTarget(self, action: #selector(clickScanBtnClickHandle(sender:)), for: .touchUpInside)
