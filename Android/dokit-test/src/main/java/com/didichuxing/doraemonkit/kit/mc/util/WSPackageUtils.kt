...
object WSPackageUtils {
    ...
    fun toPackageJson(id: String, type: PackageType, event: WSEvent): String {
        val wsPackage = WSPackage(id, type, GsonUtils.toJson(event), connectSerial = DokitConnectManager.getConnectSerial())
        return GsonUtils.toJson(wsPackage)
    }

    fun toPackageJson(id: String, type: PackageType, data: String): String {
        val wsPackage = WSPackage(id, type, data, connectSerial = DokitConnectManager.getConnectSerial())
        return GsonUtils.toJson(wsPackage)
    }
