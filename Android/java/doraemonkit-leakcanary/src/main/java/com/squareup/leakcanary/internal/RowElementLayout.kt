...
class RowElementLayout(context: Context?, attrs: AttributeSet?) : ViewGroup(context, attrs) {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val moreSizeSpec = MeasureSpec.makeMeasureSpec(moreSize, MeasureSpec.EXACTLY)
        ...
        val detailsWidthSpec = MeasureSpec.makeMeasureSpec(detailsWidth, MeasureSpec.AT_MOST)
        val detailsHeightSpec = MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED)
        details!!.measure(detailsWidthSpec, detailsHeightSpec)
        if (details!!.visibility != View.GONE) {
            totalHeight += details!!.measuredHeight
        }
        totalHeight = totalHeight.coerceAtLeast(minHeight)
        val connectorWidthSpec = MeasureSpec.makeMeasureSpec(connectorWidth, MeasureSpec.EXACTLY)
        val connectorHeightSpec = MeasureSpec.makeMeasureSpec(totalHeight, MeasureSpec.EXACTLY)
