...
import com.squareup.leakcanary.internal.SquigglySpan.Companion.replaceUnderlineSpans
...
internal class DisplayLeakAdapter(resources: Resources) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View, parent: ViewGroup): View {
        ...
        if (getItemViewType(position) == TOP_ROW) {
            ...
            val connector = findById<DisplayLeakConnectorView>(convertView, R.id.leak_canary_row_connector)
            ...
            moreDetailsView.opened = opened[position]
            if (opened[position]) {
            ...
            if (position == 1) {
                titleView.text = Html.fromHtml("<font color='"
                        + helpColorHexString
                        + "'>"
                        + "<b>" + resources.getString(R.string.leak_canary_help_title) + "</b>"
                        + "</font>")
                val detailText = Html.fromHtml(
                        resources.getString(R.string.leak_canary_help_detail)) as SpannableStringBuilder
                replaceUnderlineSpans(detailText, resources)
                ...
                maybeLeakCause = if (isLeakingInstance || reachability === Reachability.UNREACHABLE) {
                    false
                } else {
                    val nextReachability = leakTrace!!.expectedReachability[elementIndex(position + 1)]
                    nextReachability !== Reachability.REACHABLE
                }
    ...
    private fun htmlTitle(element: LeakTraceElement?, maybeLeakCause: Boolean, resources: Resources): Spanned {
        ...
        if (element?.reference != null) {
            var referenceName = element.reference.displayName.replace("<".toRegex(), "&lt;")
                    .replace(">".toRegex(), "&gt;")
        ...
        if (maybeLeakCause) {
            replaceUnderlineSpans(builder, resources)
        }
        return builder
    }

    private fun htmlDetails(isLeakingInstance: Boolean, element: LeakTraceElement?): Spanned {
        var htmlString = ""
    ...
    private fun getConnectorType(position: Int): DisplayLeakConnectorView.Type {
        return if (position == 1) {
            DisplayLeakConnectorView.Type.HELP
        } else if (position == 2) {
            ...
            if (isLeakingInstance) {
                ...
                val reachability = leakTrace!!.expectedReachability[elementIndex(position)]
                when (reachability) {
                    Reachability.UNKNOWN -> DisplayLeakConnectorView.Type.NODE_UNKNOWN
                    ...
                    else -> throw IllegalStateException("Unknown value: $reachability")
                }
            }
        }
    }

    fun update(leakTrace: LeakTrace, referenceKey: String, referenceName: String) {
    ...
    companion object {
        ...
        private fun <T : View?> findById(view: View, id: Int): T {
