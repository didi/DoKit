...
import com.squareup.leakcanary.internal.LeakCanaryInternals.Companion.showNotification
import com.squareup.leakcanary.internal.RequestStoragePermissionActivity.Companion.createPendingIntent
import java.io.File
import java.io.FilenameFilter
import java.util.*

class DefaultLeakDirectoryProvider @JvmOverloads constructor(context: Context, maxStoredHeapDumps: Int = DEFAULT_MAX_STORED_HEAP_DUMPS) : LeakDirectoryProvider {
    private val context: Context
    private val maxStoredHeapDumps: Int

    @Volatile
    private var writeExternalStorageGranted = false

    @Volatile
    private var permissionNotificationDisplayed = false
    ...
    override fun newHeapDumpFile(): File? {
        val pendingHeapDumps = listFiles(FilenameFilter { _, filename -> filename.endsWith(PENDING_HEAPDUMP_SUFFIX) })
    ...
    override fun clearLeakDirectory() {
        val allFilesExceptPending = listFiles(FilenameFilter { dir, filename -> !filename.endsWith(PENDING_HEAPDUMP_SUFFIX) })
    ...
    private fun hasStoragePermission(): Boolean {
        ...
        writeExternalStorageGranted = context.checkSelfPermission(permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED
    ...
    private fun requestWritePermissionNotification() {
        ...
        val contentText = context.getString(R.string.leak_canary_permission_notification_text, packageName)
        showNotification(context, contentTitle, contentText, pendingIntent, -0x21504111)
    }
    ...
    private fun cleanupOldHeapDumps() {
        val hprofFiles = listFiles(FilenameFilter { dir, filename -> filename.endsWith(HPROF_SUFFIX) })
...
//            Collections.sort(hprofFiles) { lhs, rhs ->
//                java.lang.Long.valueOf(lhs!!.lastModified()).compareTo(rhs!!.lastModified()) }
    ...
    init {
        require(maxStoredHeapDumps >= 1) { "maxStoredHeapDumps must be at least 1" }
