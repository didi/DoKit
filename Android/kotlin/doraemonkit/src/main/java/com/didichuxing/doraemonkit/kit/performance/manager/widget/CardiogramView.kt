 ...
 * 实现平滑移动的折线图
 */
class CardiogramView(context: Context, attrs: AttributeSet?) : View(context, attrs), Runnable {
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        canvas.save()
        val translateX = getCanvasTranslate()
     ...
     * 获取当前平移的距离
     */
    private fun getCanvasTranslate(): Float {
        return -mItemWidth * (mCurrentFrameCount / mTotalFrameCount.toFloat()) + mItemWidth * (MAX_ITEM_COUNT - mList.size)
    }
