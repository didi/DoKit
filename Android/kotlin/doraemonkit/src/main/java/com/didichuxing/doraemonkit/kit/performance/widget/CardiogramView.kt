...
class CardiogramView : View, Runnable {
    ...
    private val canvasTranslate: Float
        private get() = -mItemWidth * (mCurrentFrameCount / mTotalFrameCount.toFloat()) + mItemWidth * (MAX_ITEM_COUNT - mList.size)

    private fun drawLine(canvas: Canvas) {
        // 画波动折线
        var index = 0
        while (index < Math.min(mList.size.toFloat(), DEFAULT_ITEM_COUNT + 1)) {
