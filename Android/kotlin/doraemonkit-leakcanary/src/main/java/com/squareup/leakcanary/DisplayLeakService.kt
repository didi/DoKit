...
import com.squareup.leakcanary.AnalyzedHeap.Companion.save
 ...
 */
open class DisplayLeakService : AbstractAnalysisResultService() {
    override fun onHeapAnalyzed(analyzedHeap: AnalyzedHeap) {
        ...
        if (resultSaved) {
            val pendingIntent = DisplayLeakActivity.createPendingIntent(this, heapDump.referenceKey)
            contentTitle = if (result.failure != null) {
                getString(R.string.leak_canary_analysis_failed)
            } else {
                ...
                if (result.leakFound) {
                    ...
                    getString(R.string.leak_canary_class_no_leak, className)
                }
            }
    ...
    private fun showNotification(pendingIntent: PendingIntent?, contentTitle: String,
                                 contentText: String?) {
    ...
    private fun saveResult(heapDump: HeapDump, result: AnalysisResult): Boolean {
        val resultFile = save(heapDump, result)
    ...
    private fun renameHeapdump(heapDump: HeapDump): HeapDump {
        ...
        if (!renamed) {
            CanaryLog.d("Could not rename heap dump file %s to %s", heapDump.heapDumpFile.path,
                    newFile.path)
        }
     ...
     */
    protected open fun afterDefaultHandling(heapDump: HeapDump, result: AnalysisResult,
                                            leakInfo: String) {
    }

    companion object {
        private const val TAG = "DisplayLeakService"
    }
}