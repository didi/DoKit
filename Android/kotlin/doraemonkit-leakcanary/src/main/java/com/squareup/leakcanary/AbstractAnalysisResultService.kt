...
abstract class AbstractAnalysisResultService : ForegroundService(AbstractAnalysisResultService::class.java.name,
     ...
     * In [AnalyzedHeap.result] check [AnalysisResult.leakFound] and [ ][AnalysisResult.excludedLeak] to see if there was a leak and if it can be ignored.
     *
     *
    ...
    protected fun onHeapAnalyzed(heapDump: HeapDump, result: AnalysisResult) {
    }
    ...
    companion object {
        ...
        fun sendResultToListener(context: Context,
                                 listenerServiceClassName: String,
                                 heapDump: HeapDump,
                                 result: AnalysisResult) {
            val listenerServiceClass: Class<*>
            listenerServiceClass = try {
                Class.forName(listenerServiceClassName)
